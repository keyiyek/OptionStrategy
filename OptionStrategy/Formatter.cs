...
namespace OptionStrategy
{
    ...
    {
        // Some general storage Array
        double[] probabilityDiffLine;
        double[] averageList;
        double currentPrice;
        ...
        // All the other lines
        public double[] NormalLine(double[,] resultTable, int line)
        {
            // Set up some local variables
            double lineAverage = 0; // Caverage that goes at the end
            double[] resultsLine = new double[resultTable.GetUpperBound(0) + 5]; // +5 because we have one more element at the end: the Average
            ...
            resultsLine[2] = resultTable[line, 0] - resultTable[line, 2]; // Third column is the BE Price (Strike - Bid)

            // for all the Strike Prices
            for (int i = 3; i <= resultsLine.GetUpperBound(0) - 1; i++)
            {
                ...
                {
                    resultsLine[i] = (((resultTable[i - 3, 0] - 0.5) - resultTable[line, 0]) + resultsLine[1]) * probabilityDiffLine[i];
                }
